---
title: "assignment 2_ariana marastuti"
author: "Tamas Nagy"
date: '2022-05-03'
output:
...
```{r}
phd_field_raw <- readr::read_csv("https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2019/2019-02-19/phd_by_field.csv")
str(phd_field_raw)
aggregate_phd <- phd_field_raw %>%
...
head(commute_mode_raw)
str(commute_mode_raw)
aggregate_commute <- commute_mode_raw %>%
  group_by(state_region, state_abb, mode) %>%
  summarise(n = sum(n, na.rm = TRUE))
aggregate_bike <- aggregate_commute %>%
  group_by(state_region, state_abb, n) %>%
  filter(mode == "Bike") %>%
  rename(n_biker = n)
aggregate_walk <- aggregate_commute %>%
  group_by(state_region, state_abb, n) %>%
  filter(mode == "Walk") %>%
  rename(n_walker = n)
commute_tot <- cbind(aggregate_walk, aggregate_bike)
commuter <- list(aggregate_walk, aggregate_bike) %>% 
