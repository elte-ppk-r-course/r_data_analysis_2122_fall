...
titanic_pass <- read.spss("C:\\Users\\Lenovo\\Desktop\\Git project\\r_data_analysis_2122_fall\\assignments\\data\\assignment_6_dataset.sav")

titanic_pass %>% 
  knitr::kable()
glimpse(titanic_pass)
```
...
```{r}
class(titanic_pass$Age)

unique(titanic_pass$Age)
```


```{r}
sum(is.na(titanic_pass$PassengerId))
sum(is.na(titanic_pass$Survived))
sum(is.na(titanic_pass$Pclass))
sum(is.na(titanic_pass$Name))
sum(is.na(titanic_pass$Sex))
sum(is.na(titanic_pass$Age))
sum(is.na(titanic_pass$SibSp))
sum(is.na(titanic_pass$Parch))
sum(is.na(titanic_pass$Ticket))
sum(is.na(titanic_pass$Fare))
sum(is.na(titanic_pass$Cabin))
sum(is.na(titanic_pass$Embarked))

```
Changing the missing value of age variable with mean. 

```{r}
mean(titanic_pass$Age, na.rm = TRUE)
age_median <- median(titanic_pass$Age, na.rm = TRUE)
titanic_pass[is.na(titanic_pass$Age), "Age"] <- 29

```
```{r}

titanic_pass <- titanic_pass%>%
