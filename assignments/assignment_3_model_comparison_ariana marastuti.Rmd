...
pain <- read_excel("C:/Users/Lenovo/Downloads/assignment_3_dataset (1).xlsx")
...
simple_reggmod <- lm(pain ~ age + sex, data = pain)

nrow(model.frame(simple_reggmod))

summary(simple_reggmod)
```
...
abline(h = 3*mean(cooksd,na.rm = T)), col="green")
text(x=1:length(cooksd)+1, y=cooksd, labels=ifelse(cooksd>4*mean(cooksd, na.rm=T),names(cooksd),""), col="green")
```
