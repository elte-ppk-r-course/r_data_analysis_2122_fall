...
```{r}
url <- "https://raw.githubusercontent.com/elte-ppk-r-course/r_data_analysis_2122_fall/master/data/assignment_5_dataset.csv"
animalrights <- read.csv(url)
```
...
```{r}
KMO(ar)
#The overall KMO is 0.88, which means that the dataset is adequate for the factor analysis.
...
scree(ar, factors = FALSE)

#Based on the scree plot we can see that the recommended factor number is 6, but I would go with 5 because the 6th factor goes a bit lower compared to the line.
```
...
```{r}
fa_ar <- fa(ar, factors = 5, rotate= "varimax")
fa_ar
```
...
```{r}
sort(fa_ar$communality, decreasing = TRUE)
```
...
mean(fa_ar$communality)

#The mean communality scores are 0.295.
...
#Based on the factor analysis I would remove the following items: ar19, ar21, ar24, ar16 and ar18.
...
```{r}
logisticr <- glm()
#I was not sure how to do the last part properly.
```
